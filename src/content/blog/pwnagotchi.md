...
To do this, it has to prove it knows the password without actually sending the password in plain text. It performs a 4-Way Handshake. The Pwnagotchi waits for that moment of reconnection and records the the Handshake between your phone and the router. This handshake contains a hash derived from the real password.
...
![PiSugar 3](../../assets/pwnagotchi/pisugar.jpg)
  - **⚠️ Warning:** Sometimes the golden pins on the **PiSugar 3** don't make enough contact to the **Raspberry Pi** and won't power up the unit. This is where the soldering comes in. You may need to apply a little bit of solder to the corresponding GPIO pins on the backside of the **Raspbery Pi**
...
---
### Preparation
- There's no real setup you need to do with Linux. As of this writing I'm currently using Linux Mint and all you need to do is plug in your USB-A to Micro USB data cable into the micro usb port closet to the HDMI port. 
...
- Once the Pwnagotchi is done setting up, you should see a new **USB Ethernet** connection under your Network settings
![Linux - USB Ethernet](../../assets/pwnagotchi/linux_usb_ethernet.png)
...
---
- Configuration of the Pwnagotchi is the same whether you're using Linux or Windows, since will be doing everything through SSH.
...
- Now you should be connected to the Pwnagtochi and can run the wizard by running ```sudo pwnagotchi --wizard```
- Follow the prompts